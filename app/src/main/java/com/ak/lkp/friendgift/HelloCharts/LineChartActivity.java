...
public class LineChartActivity extends AppCompatActivity {
    ...
    int[] score= {50,42,30,33,10,25,22,18,36,-20};//图表的数据点
    int[] score1= {-20,5,15,33,15,-15,22,18,30,40};//图表的数据点
    int[] score2= {30,20,22,28,-10,30,12,-18,14,8};//图表的数据点
    ...
    private void initLineChart(){
        ...
        Line line1 = new Line(mPointValues1).setColor(Color.parseColor("#ee9933"));  //折线的颜色（橙色）
        Line line2 = new Line(mPointValues2).setColor(Color.parseColor("#9933ee"));  //折线的颜色（橙色）
...
//      line.setHasLabelsOnlyForSelected(true);//点击数据坐标提示数据（设置了这个line.setHasLabels(true);就无效）
        ...
        line1.setPointRadius(2);//座标点大小
        line1.setStrokeWidth(1);//线的粗细
//      line.setHasLabelsOnlyForSelected(true);//点击数据坐标提示数据（设置了这个line.setHasLabels(true);就无效）
        line1.setHasLines(true);//是否用线显示。如果为false 则没有曲线只有点显示
        line1.setHasPoints(true);//是否显示圆点 如果为false 则没有原点只有点显示（每个数据点都是个大的圆点）
        lines.add(line1);

        line2.setShape(ValueShape.SQUARE);//折线图上每个数据点的形状  这里是圆形 （有三种 ：ValueShape.SQUARE  ValueShape.CIRCLE  ValueShape.DIAMOND）
        line2.setCubic(false);//曲线是否平滑，即是曲线还是折线
        line2.setFilled(false);//是否填充曲线的面积
        line2.setHasLabels(true);//曲线的数据坐标是否加上备注
        line2.setPointRadius(4);//座标点大小
        line2.setStrokeWidth(2);//线的粗细
//      line.setHasLabelsOnlyForSelected(true);//点击数据坐标提示数据（设置了这个line.setHasLabels(true);就无效）
